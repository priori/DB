<?php 
	...
	class Model{
		...
		private function get_col_name( &$a ){
			return isset($a['col'])?$a['col']:(isset($a['name'])?$a['name']:false);
		}
		private function get_alias_name( &$a ){
			return isset($a['alias'])?$a['alias']:(isset($a['name'])?$a['name']:false);
		}
		private function is_col( &$attr ){
			return isset($attr['!']) && !isset($attr['model']) && !is_array($attr['content']) || 
		...
		private function add_val( &$q, &$attr, &$default_entry ){
			...
			if( isset($attr['sql']) ){
				...
				if( isset($attr['serialize']) ){ 
					// || is_object($attr['serialize']) || is_array($attr['serialize'])
					// content array poderiam estar maculados pela funcao que decodifica as macros
		...
		public function __add(&$e,$replace=false){
				...
				// is array, sei lá!
					...
					foreach( $attr as $attr2 ){
						...
						// }elseif( is special ){
							...
							die('nao aceita macros complicadas para insersoes multiplas'.
								' que nao na primeira insersao');
							// $actual_entry['!'][] =& $attr2;
			...
			if( $fazer_por_partes ){
				...
				foreach( $entries as $c => $e ){
					...
					$r0 = $this->_values( $names, $es, $q, $need_transaction, $specials[$c], 
													$aux, $default_special );
				...
				return $this->_values( $names, $entries, $q, $need_transaction, $a, $default_entry, $default_special );
		...
		private function _values( &$names, &$entries, &$q, &$need_transaction, &$special, 
				&$default_entry, &$default_special ){
			...
			if( $need_transaction ){
				...
				foreach( $special as $attr ){
					...
					if( is_array($attr['content']) && $n ){
						...
						if( isset($attr['fk']) )
							$fk = $attr['fk'];
						$attr['content'][] = array('!'=>true,'col'=>$fk,'content'=>''.$id ,'name'=>$fk);
